local self = {}
GCAD.RealFIRFilter = GCAD.MakeConstructor (self)

-- Y / X = b0 + b1 z^-1 + b2 z^-2 + b3 z^-3 + b4 z^-4 + ...
-- y_n = b0 x_n + b1 x_n-1 + b2 x_n-2 

function GCAD.RealFIRFilter.FromImpulseResponse (...)
	local firFilter = GCAD.RealFIRFilter ()
...
function self:CreateCompiledFilter ()
	if not self.FactoryValid then
		...
		local compiled = CompileString (self.FactoryCode, "GCAD.RealFIRFilter", false)
		if isstring (compiled) then
			GCAD.Error ("RealFIRFilter:CreateCompiledFilter : Failed to compile filter (" .. compiled .. ")")
